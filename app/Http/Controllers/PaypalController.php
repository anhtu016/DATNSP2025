...
class PaypalController extends Controller
{
    ...
    public function success(Request $request)
    {
        ...
        try {
            ...
            $paidCart = $checkoutData['cart'] ?? [];

            // Xóa các sp đã thanh toán khỏi giỏ hàng
            foreach ($paidCart as $id => $item) {
                unset($cart[$id]);
            }

            // Cập nhật lại giỏ hàng còn lại trong session
            session(['cart' => $cart]);

            // Xóa các session liên quan
            session()->forget(['coupon', 'checkout_data']);
